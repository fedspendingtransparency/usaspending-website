    ...
    useEffect(() => {
        const renderBarChart = () => {
            ...
            if (numerator.className.includes("asst-non-federal-funding")) {
                ...
                if (!isNffZero) {
                    ...
                    chartSvg.append("foreignObject")
                        ...
                        .attr('x', 0)
                        .attr('y', 230)
                        .html(`<div className="award-amounts-viz-label__desc-text current"><strong>${currentAmountValue}</strong><br />${currentAmountLabel}</div>`)
                        .select('div')
                        .style('float', 'right')
                ...
                if (obligatedAmountValue.indexOf("$0") < 0) {
                    chartSvg.append("foreignObject")
                        ...
                        .attr('y', 90)
                        .html(`<div className="award-amounts-viz-label__desc-text blah2"><strong>${obligatedAmountValue}</strong><br />${obligatedAmountLabel}</div>`)
                        .select('div')
                        .style('float', x(propsArr[0]) - x(propsArr[2]) <= 270 ? 'right' : 'left')
                        .style('text-align', x(propsArr[0]) - x(propsArr[2]) <= 270 ? 'right' : 'left')
                        .style('background-color', 'white')
            ...
            chartSvg.append("foreignObject")
                ...
                .attr('x', 0)
                .attr('y', 300)
                .html(`<div className="award-amounts-viz-label__desc-text blah"><strong>${potentialAmountValue}</strong><br />${potentialAmountLabel}</div>`)
                .select('div')
                .style('float', 'right')
                .style('text-align', 'right')
                .select('strong')
        ...
        const renderBarChartLoans = () => {
            ...
            if (currentAmountValue?.indexOf("$0") < 0) {
                chartSvg.append("foreignObject")
                    ...
                    .attr('x', x(propsArr[0]) - x(propsArr[1]) <= 270 ? 0 : x(propsArr[1]) + 10)
                    .attr('y', 90)
                    .html(`<div className="award-amounts-viz-outlays__desc-text subsidy"><strong>${currentAmountValue}</strong><br />${currentAmountLabel}</div>`)
                    .select('div')
                    .style('float', x(propsArr[0]) - x(propsArr[1]) <= 270 ? 'right' : 'left')
                    .style('text-align', x(propsArr[0]) - x(propsArr[1]) <= 270 ? 'right' : 'left')
                    .select('strong')
            ...
            chartSvg.append("foreignObject")
                ...
                .attr('x', 0)
                .attr('y', 230)
                .html(`<div className="award-amounts-viz-outlays__desc-text facevalue"><strong>${potentialAmountValue}</strong><br />${potentialAmountLabel}</div>`)
                .select('div')
                .style('float', 'right')
                .style('text-align', 'right')
                .select('strong')
