/**
 * Created by michaelbray on 5/25/17.
 */

// Disabling max-len property for readability / editability
/* eslint-disable max-len */

const productionURL = 'https://usaspending.gov/';
const imgDirectory = 'img/';

const facebookImage = 'FacebookOG.png';
const twitterImage = 'TwitterCard.png';

export const homePageMetaTags = {
    og_url: productionURL,
    og_title: "USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: productionURL,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const searchPageMetaTags = {
    og_url: `${productionURL}#/search`,
    og_title: "Advanced Search - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Advanced Search - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/search`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const explorerPageMetaTags = {
    og_url: `${productionURL}#/explorer`,
    og_title: "Spending Explorer - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Spending Explorer - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/explorer`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const aboutPageMetaTags = {
    og_url: `${productionURL}#/about`,
    og_title: "About - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "About - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/about`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const accessibilityPageMetaTags = {
    og_url: `${productionURL}#/about/accessibility`,
    og_title: "Accessibility - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Accessibility - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/about/accessibility`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const privacyPageMetaTags = {
    og_url: `${productionURL}#/about/privacy`,
    og_title: "Privacy - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Privacy - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/about/privacy`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const foiaPageMetaTags = {
    og_url: `${productionURL}#/about/foia`,
    og_title: "Freedom of Information Act - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Freedom of Information Act - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/about/foia`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const dbInfoPageMetaTags = {
    og_url: `${productionURL}#/db_info`,
    og_title: "Dun & Bradstreet, Inc. Data - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Dun & Bradstreet, Inc. Data - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/db_info`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const awardPageMetaTags = {
    og_url: `${productionURL}#/award/`,
    og_title: "Award Summary - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Award Summary - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/award/`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const federalAccountPageMetaTags = {
    og_url: `${productionURL}#/federal_account/`,
    og_title: "Federal Account Profile - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Federal Account Profile - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/federal_account/`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const agencyPageMetaTags = {
    og_url: `${productionURL}#/agency/`,
    og_title: "Agency Profile - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Agency Profile - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/agency/`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const agencyLandingPageMetaTags = {
    og_url: `${productionURL}#/agency`,
    og_title: "Agency Profiles - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Agency Profiles - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/agency`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const stylePageMetaTags = {
    og_url: `${productionURL}#/style`,
    og_title: "Style Guide - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Style Guide - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/style`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const downloadArchivePageMetaTags = {
    og_url: `${productionURL}#/download_center/award_data_archive`,
    og_title: "Award Data Archive - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Award Data Archive - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/download_center/award_data_archive`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const downloadAwardPageMetaTags = {
    og_url: `${productionURL}#/download_center/custom_award_data`,
    og_title: "Custom Award Data - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Custom Award Data - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/download_center/custom_award_data`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const downloadAccountPageMetaTags = {
    og_url: `${productionURL}#/download_center/custom_account_data`,
    og_title: "Custom Account Data - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Custom Account Data - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/download_center/custom_account_data`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const dataDictionaryPageMetaTags = {
    og_url: `${productionURL}#/download_center/data_dictionary`,
    og_title: "Data Dictionary - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Data Dictionary - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/download_center/data_dictionary`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const keywordPageMetaTags = {
    og_url: `${productionURL}#/keyword_search`,
    og_title: "Keyword Search - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Keyword Search - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/keyword_search`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const statePageMetaTags = {
    og_url: `${productionURL}#/state/`,
    og_title: "State Profile - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "State Profile - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/state/`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const errorPageMetaTags = {
    og_url: productionURL,
    og_title: "404 - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "404 - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: productionURL,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const recipientPageMetaTags = {
    og_url: `${productionURL}#/recipient/`,
    og_title: "Recipient Profile - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Recipient Profile - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/recipient/`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const accountLandingPageMetaTags = {
    og_url: `${productionURL}#/federal_account`,
    og_title: "Federal Account Profiles - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Federal Account Profiles - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/federal_account`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const recipientLandingPageMetaTags = {
    og_url: `${productionURL}#/recipient`,
    og_title: "Recipient Profiles - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "Recipient Profiles - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/recipient`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

export const stateLandingPageMetaTags = {
    og_url: `${productionURL}#/state`,
    og_title: "State Profiles - USAspending.gov",
    og_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    og_site_name: "USAspending.gov",
    og_image: `${productionURL}${imgDirectory}${facebookImage}`,
    twitter_title: "State Profiles - USAspending.gov",
    twitter_description: "USAspending.gov is the new official source of accessible, searchable and reliable spending data for the U.S. Government.",
    twitter_image: `${productionURL}${imgDirectory}${twitterImage}`,
    twitter_url: `${productionURL}#/state`,
    twitter_label1: "",
    twitter_data1: "",
    twitter_label2: "",
    twitter_data2: ""
};

/* eslint-enable max-len */
