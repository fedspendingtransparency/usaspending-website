...
describe('AgenciesContainer', () => {
    ...
    describe('table loads the correct fy by url', () => {
        it('shows table for fy from url', async () => {
            useParams.mockImplementation(() => ({ fy: '2019', period: '12' }));
            render(<AboutTheDataPage history={history} />);
            await waitFor(() => {
                const picker = screen.getByTestId('selected-fy');
                expect(picker.innerHTML).toEqual('FY 2019');
            });
        });
        it('when fy in url is not valid the latest fy is displayed and url is updated', async () => {
            useParams.mockImplementation(() => ({ fy: '2012', period: '12' }));
            render(<AboutTheDataPage history={history} />);
            await waitFor(() => {
                const picker = screen.getByTestId('selected-fy');
                expect(picker.innerHTML).toEqual('FY 2020');
            });
            expect(mockPush).toHaveBeenCalledWith({
                pathname: '/about-the-data/agencies/2020/12'
            });
        });
    });
});
