...
describe('Line Helper', () => {
    it('should return null if no date', () => {
        expect(validateLineValue()).toBeNull();
    });
    it('should return null if date isNaN', () => {
        expect(validateLineValue(moment(''))).toBeNull();
    });
    it('should return a number if date exists', () => {
        expect(validateLineValue(goodDates._startDate, [goodDates._startDate.subtract(7, 'd'), goodDates._startDate.add(7, 'd')])).toBe(goodDates._startDate.valueOf());
    });
    it('should return null when date is less than domain', () => {
        expect(validateLineValue(3, [10, 20])).toBeNull();
    });
    it('should return null when date is greater than domain', () => {
        expect(validateLineValue(25, [10, 20])).toBeNull();
    });
});
