...
1. Use nvm to install and use Node 6.11.0.
	1. Install nvm by running this command in terminal `curl -o- https://raw.githubusercontent.com/creationix/nvm/v0.33.8/install.sh | bash`
	2. Confirm you have a bash_profile in your home directory  (In terminal run `ls -a` and make sure that the file `~/.bash_profile` is present. If not run `touch ~/.bash_profile`) then run `export NVM_DIR="$HOME/.nvm" [ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"`
	3. Run `nvm --version` to confirm successful installation
	4. Install and use node 6.11.0 with `nvm install 6.11.0 && nvm use 6.11.0`
	5. Run `node -v` and confirm that you are now using node 6.11.0
